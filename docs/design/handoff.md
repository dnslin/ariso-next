...
4. **保留 Ariso 视觉与已确认交互。** 使用主题和样式配置保留品牌黄、水绿、字体、双柔光、信息层级及响应式布局。通用控件的内部间距、边框、圆角和状态表现可按 HeroUI 统一，不要求逐像素复刻原型中的手绘细节。固定底栏、手机全屏导航、字段外标签等明确约定继续执行，不能为套用组件擅自改变。
5. **原型只在必要时调整。** 业务表达不清、状态遗漏或实际使用受影响时，修正对应原型；组件细节调整记录在任务或 PR 中即可。涉及业务行为、页面结构或已确认交互的变化，先明确差异并更新对应设计或规格，不能以“采用 HeroUI”为由直接覆盖。
