...
后台导航、品牌区、当前项表现、用户区和手机菜单由共享组件及其统一配置维护。页面只传入业务内容和必要上下文，不各自复制导航清单、遗漏图标或重排公共区域。面包屑同样由公共外壳提供；同一视口下固定起点、容器宽度、行高和标题间距，不因路由、加载或业务状态变化而跳动，也不要求脱离正文使用 CSS fixed。新增入口时更新公共来源，并检查已有消费页面；只复用外壳容器、不统一其中的公共内容，不算完成复用。不为此另建通用 UI 框架。
