...
- 版本切换使用 8px 圆角，选中版本使用品牌黄。详情及复制流程内可见返回文案统一“返回”，无障碍名称保留目的地；回收记录底栏不重复返回按钮。
- 权限说明使用 HeroUI Popover，由“公开／私有”或“仅管理员可见”按钮展开，支持键盘、触屏与 Escape 回焦。常规规则不常驻横幅；真实错误、禁用原因和复制／下载原图的必要提示仍就近呈现。
- 上传主内容最大 1200px；768px 起设置列 360px、间隔 24px，两卡最小高度 360px；手机单列，上传卡最小 280px。上传选择图标提供细微悬停反馈，上传中才显示动态状态，成功图标短暂淡入；尊重减少动态效果。
- 公共底栏的背景与分隔线铺满主区域，内部边距与正文一致：桌面 32px、手机和平板 16px，并保留安全区。首次进入回收站不主动聚焦标题；从记录返回或恢复后仍管理焦点。

对应桌面／手机节点：图库详情 `36:312/102:3228`，上传 `30:97/101:1014`，回收列表 `30:1037/102:852`，回收记录 `405:6888/405:6735`。实际读取、Figma 同步、浏览器和独立评审证据汇入 [M2 本轮 UI 记录](../verification/m2-85/ui-refinement/README.md)。最终仍需用户人工验收。
