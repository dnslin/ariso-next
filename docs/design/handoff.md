...
后台导航、品牌区、当前项表现、用户区和手机菜单由共享组件及其统一配置维护。页面只传入业务内容和必要上下文，不各自复制导航清单、遗漏图标或重排公共区域。新增入口时更新公共来源，并检查已有消费页面；只复用外壳容器、不统一其中的公共内容，不算完成复用。不为此另建通用 UI 框架。

临时占位和部分交付必须在当前任务证据中写明未实现范围、承接任务和替换条件。未实现入口如何呈现以用户确认的设计为准，不在此规定统一显隐策略，也不把临时入口或占位当作最终设计。后续任务消费该区域时复核替换条件，条件满足即移除对应占位；不能因早期夹具或占位曾通过检查而一直沿用。
