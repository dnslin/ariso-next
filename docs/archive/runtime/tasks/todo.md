...
- 模块：`runtime`。
- 状态：RUNTIME-01–19 已合入；RUNTIME-20–26 已实现并通过本批验收，整合 PR 待评审。最新结果见 [runtime 验证记录](../runtime-verification.md)，实施状态与实际验收分别维护。
- 日期：2026-09-12。
- 依据：[runtime Spec](../SPEC-runtime.md)、[实现计划](./plan.md)、[能力地图](../../../product/CAPABILITY-MAP.md)。
- 用户已批准整批实施 #20–#26，允许一个整合 PR、按任务拆分提交；本轮不合并、不关闭 Issue、不发布镜像。

## 执行约定

任务按依赖执行。每项完成后记录实际命令、退出结果及证据位置，再勾选完成。文档通过校验不代表其中的应用测试已经通过。预计文件是实施范围；发现一项需要超过约五个文件或一次专注工作时，先继续拆分，不能用目录通配符隐藏任务规模。

下列命令从项目根目录、在 RUNTIME-01 确定的 Node 24 环境执行。测试命令及脚本已随对应任务实现。初始工程使用直接的 Next 命令；CLI 与打包就绪后在 RUNTIME-09 落实 Spec 的完整命令契约。测试脚本只在真实测试存在时接入。
...
- [x] Node 24 和 pnpm 11.19.0 的实际路径、版本与使用方法有记录；已记录 Docker/buildx 及双架构构建与验证由 GitHub Actions 执行，本机不作要求。
- [x] 已批准且首批会使用的依赖完成安装；原生构建脚本按实际依赖图配置；冻结锁文件安装成功。
- [x] better-sqlite3 在目标 Node 中实际执行查询；密钥、本地数据和生成产物不进入 Git。
...
- [x] Spec 的 lint 与格式命令可执行，冻结 PRD 和生成目录被排除。
- [x] 应用类型检查与真实 Next 构建成功；工作流使用目标 Node 和冻结锁文件。
- [x] CI 仅执行当前可用检查，并说明后续接入点；远端未运行时保留未验证状态。

**验证：** `pnpm run lint`；`pnpm run format:check`；`pnpm run typecheck`；`pnpm exec next build`。存在远端时记录实际工作流结果，不以 YAML 存在代替运行。

### 检查点 K1（计划 C1）

- [x] RUNTIME-01–03 的目标版本安装、页面、静态资源、无密钥构建和基础检查证据齐备。
- [x] 汇报第一个可运行工程；未具备的 Docker/远端环境单列，不影响本地 R2 推进。
...
- [x] 缺失/非法密钥、非法端口、不可写目录和故障迁移均非零退出，轮询确认未启动 Web；生产端口占用时不自动换端口。
- [x] 旧产物面对新数据库退出，未删除或降级已有数据；修复故障后启动同一目录成功。
- [x] 通过完整入口停止、重启后读到此前写入的测试记录；进程失败不留下测试服务或共享数据。
...
**实施记录（2026-09-15）：** 已补充 12 项完整入口失败与恢复测试，并与 RUNTIME-09 复用进程辅助函数。真实目录权限、端口占用、SQL 回滚、新旧迁移进度及停止重启后的数据保留均已本地验证；没有修改生产实现或新增业务表。完整检查、审计和远端证据见 [开发记录](../development.md#runtime-10完整入口的失败与恢复)。CI 与 AMD64/ARM64 Docker 检查均通过，PR #36 已合入，对应 Issue 已由用户关闭。
...
- [x] 子进程删除两个秘密变量，使用不存在的 DATA_DIR，完整生产构建成功且未写数据、未输出初始化码。
- [x] 构建测试使用独立输出目录，测试并发不会改写其他产物测试正在运行的目录。
- [x] 当前真实单元、构建和集成检查已接入 CI；尚无浏览器测试的部分留给 RUNTIME-23。
...
- [x] 正常生产产物经 HTTP 返回 200、no-store，未初始化站点也可用；没有外部存储请求。
- [x] 临时测试应用复用实际健康处理器和数据库连接，经测试专用组合关闭真实连接后，HTTP 返回 503 且保留可诊断日志；故障入口不进入生产产物。
- [x] 重复调用初始化及模拟模块重新加载不会重复创建连接；有限初始化可完成返回。
...
- [x] R2 的正常启动、故障退出、迁移、持久化、HTTP 503、无副作用构建和隔离目录证据齐备。
- [x] 当前单元、集成、类型与 lint 检查成功；尚未实现的密钥预检和 JSON 日志不提前验收。

**实施记录（2026-09-15）：** 本地 55 项单元、55 项集成、完整构建、类型、lint、格式及 Ego Lite 200/503 验证通过。独立审计和远端检查结果见 [开发记录](../development.md#runtime-12http-故障与初始化复用)。
...
- [x] 完成实现、本地验证和独立审计；CI 与双架构 Docker 均通过，已合入 main。
...
- [x] 普通 Node 测试组合复用实际迁移和加解密实现，读取真实测试密文；错误密钥阻止入口继续运行 Web，错误指明配置位置。
- [x] 迁移已成功后预检失败，已提交迁移保留；修复密钥后可继续启动，空生产数据库使用合法密钥正常启动。
- [x] 测试表和组合只在临时样本存在；生产不增加通用配置表、未知字段扫描或通用提供方注册框架。S3/SMTP/OAuth 字段接入仍待所属模块验收。

**验证：** `pnpm run build`；`pnpm exec vitest run --project integration tests/integration/runtime/secret-preflight.test.ts`。

**实施记录（2026-09-15）：** 4 项聚焦启动测试、70 项单元与 66 项集成测试通过；lint、格式、类型、生产构建及 Ego Lite 冒烟通过。独立审计无阻塞发现。普通同步组合复用现有迁移与加解密，错误阻止 Web 启动，修复后继续且保留迁移进度。远端证据与业务边界见 [开发记录](../development.md#runtime-14持久化秘密的启动预检)。
...
- [x] 每行日志包含时间、级别、模块和消息，错误保留阶段、路径与底层原因。
- [x] Spec 指定的认证头、Cookie、Token、凭据和启动密钥在结构化字段中隐藏；已知 URL 查询凭据在字符串和错误中隐藏。
- [x] 非敏感路径、查询参数和错误原因保留；处理显式规则，不引入通用秘密扫描服务。
...
**实施记录（2026-09-15）：** 日志核心与显式脱敏已实现，28 项聚焦测试、66 项集成及本地工程检查通过。独立审计、CI 和 AMD64/ARM64 Docker 构建与容器验证通过；真实框架输出仍由 RUNTIME-16–17 验收。见 [开发记录](../development.md#runtime-15结构化日志与已知-url-脱敏)。

### 检查点 K5

- [x] RUNTIME-13–15 的密文、失败预检和日志单元证据齐备，生产与测试样本的边界明确。
- [x] 核心日志测试通过只算局部结果，实际框架日志留待下一检查点。
...
- [x] 完成实现、本地验证与独立审计；CI 和双架构 Docker 均通过，已合入 main。
...
- [x] 最终产物的启动、正常事件和错误输出逐行可解析为 JSON，诊断上下文完整。
- [x] 临时测试应用复用实际日志入口，带重置 Token 的请求触发 Next 自身的请求 URL 错误输出；全部指定秘密均不出现，非敏感信息仍可定位。
- [x] 失败用例确实触发框架错误，不能因没有产生日志而通过；测试故障路由未进入生产应用。
...
**实施记录（2026-09-16）：** 新增 3 项进程测试，聚焦 7 项、全量 104 项单元与 70 项集成通过；lint、格式、类型、生产构建及 Ego Lite 冒烟通过。临时 Next 应用触发自身 URL 解析错误，验证非空框架诊断和指定秘密脱敏，无新增依赖或生产故障路由。审计、远端证据和健康日志既有边界见 [开发记录](../development.md#runtime-17真实框架日志验证)。

### 检查点 K6（计划 C3）

- [x] R3 的单元结果与真实入口输出一致，已有密文失败行为和框架 URL 日志均有进程证据。
- [x] 当前完整检查成功；记录 RT-08 仍需业务字段接入的部分。
...
- [x] 构建与运行使用批准的 Node/Debian 基线；原生依赖在目标架构安装，所有指定图片包和字体进入最终镜像。
- [x] 数据、秘密和开发生成物不进入构建上下文；构建无需运行密钥，运行时不联网安装依赖。
- [x] 最终 /app 包含实际产物、迁移和小型真实图片样本；样本来自项目自建或授权来源，未作为 Web 静态资源公开。

**验证：** `docker build --tag ariso:runtime .`；检查最终目录和软件版本；后续 RUNTIME-19–20 验证实际处理与服务行为，不把安装成功当作完整验收。

**实施记录（2026-09-16）：** 镜像依赖、构建上下文排除和真实样本打包已实现，104 项单元、70 项集成及 Ego Lite 验证通过；独立审计无阻塞发现，CI 与原生 AMD64/ARM64 Docker 均成功，PR #44 已合入。实际命令、样本来源、审计与远端结果见 [开发记录](../development.md#runtime-18实际运行镜像)。

### RUNTIME-19：用真实图片验证镜像工具

- [x] 完成实现、本地验证与独立审计；CI 和双架构真实图片验证通过，已合入 main。
...
- [x] 脚本实际执行 SQLite 查询、IM7 和 ExifTool，JPEG/PNG 可生成可读取的 WebP、JPEG、AVIF。
- [x] 指定字体生成中文与拉丁文字，检查文字内容可见且非空白/缺字方框；不只检查格式名称或退出码。
- [x] 从镜像 verification/fixtures 读取输入，临时输出退出后清理；无需密钥、部署数据或 /app 写权限。
...
**实施进度（2026-09-16）：** 脚本、依赖追踪、像素/字形失败测试和 Actions 样本导出已实现；109 项单元、71 项集成及本地构建和 Ego Lite 验证通过。原生 AMD64/ARM64 Actions 已完成真实转换与字形检查，导出整句图已目视核对，见 [开发记录](../development.md#runtime-19真实图片与字体验证)。

### 检查点 K7

- [x] RUNTIME-18–19 在一个实际架构上完成工具和样本验证，记录架构与执行方式。
- [x] AMD64 与 ARM64 均已在各自原生 runner 实际执行；结果分别记录，不以单架构推断另一架构。
...
- [x] Compose 显式读取变量文件；容器固定 DATA_DIR=/data、HOST=0.0.0.0、PORT=3000，默认环回映射和 30 秒停止等待符合 Spec；Node fetch 健康检查实际可用。验收脚本只使用自建临时配置、挂载与独立项目名。
- [x] 脚本检查一个长期 Node Web 进程、JSON 日志和静态资源；写入真实测试记录，停止并使用原挂载重启，记录仍可读且没有留下额外进程。
- [x] 最终镜像使用临时迁移样本验证失败不启动、重复执行及旧/新进度行为，未改写生产镜像；脚本退出清理自建容器和临时资源。
...
- [x] 两个平台分别完成工具、可见文字、原生数据库、服务启动及重启验证，记录原生或模拟执行方式。
- [x] --platform 明确选择实际目标镜像；只构建 manifest 或只模拟数据库返回不能算通过。
- [x] 记录两份结果及产物标识；APNG、动态 AVIF 和完整格式矩阵仍由 media 验收。
...
- [x] 两个平台的工具、文字、容器、停止/重启和失败入口证据齐备。
- [x] 未验证的平台或 Linux 条件明确保留；业务任务恢复未被算作 runtime 已完成。
...
**实际文件：** `scripts/verify-browser.mjs`、`e2e/runtime.mjs`、`e2e/runtime.md`、`package.json` 与锁文件；删除旧 Playwright spec。

**验收：**

- [x] ego 验收步骤可复现；生产服务使用临时目录和临时密钥，不复用用户已有服务，不下载配套浏览器。
- [x] 通过 ego 实际访问页面、健康接口和静态资源，断言可见内容与成功响应。
- [x] 应用尚无初始化、登录和上传，因此不创建跳过的业务假测试；浏览器失败保留报告并清理测试进程。
...
- [x] PR 执行 Spec 第 12.3 节的七条 CI 命令，任一失败则检查失败；另外附 ego E2E 验证记录。
- [x] CI 使用目标 Node；ego E2E 记录实际浏览器环境与覆盖范围，不假定 Actions 支持 Ego Lite；其余浏览器兼容性按 PRD 单独记录。
- [x] 有远端时记录实际 CI 结果；没有远端时只记录本地结果与工作流待运行，不标 RT-14 完成。
...
- [x] 涉及镜像、原生依赖或图片工具的 PR 均触发两架构运行，不能只在发布后检查；每个平台执行 RUNTIME-21 的工具和容器断言。
- [x] 版本发布流程面向 ghcr.io/dnslin/ariso-next，组合已经测试的架构产物并关联代码版本；不在发布步骤重新构建未经测试的替代产物。
- [x] 记录实际镜像验证工作流结果。创建远程仓库、配置凭据和实际推送镜像不在本任务执行范围；这些条件缺失时准确记录未执行部分。

**验证：** 复用 RUNTIME-21 的构建和运行命令，对照实际 CI 每个架构的报告；检查发布任务消费的产物与前置测试产物一致。没有实际发布记录时，不宣称镜像已发布。

### 检查点 K9

- [x] RUNTIME-22–24 的真实浏览器和工作流证据可追溯；本地通过与远端通过分别记录。
- [x] 所有应用检查可执行，外部环境缺失没有被静默跳过或替换成成功。
...
- [x] 从目标 Node、安装、独立密钥生成、开发到容器启动均有完整命令；区分本机 DATA_DIR 和容器 /data，说明正式 Docker 部署。
- [x] 升级前停止写入并备份整个数据目录；迁移后回滚必须恢复备份，没有自动降级或恢复空库的说明。
- [x] 使用临时部署实际演练安装、停止、备份、升级样本及备份恢复；文档只声称当前已实现能力，记录后续模块边界。
...
- [x] 每条 RT 均有对应任务、实际命令、结果与证据；失败、未执行和下游负责部分单列。
- [x] 最终检查针对同一份待交付代码和产物；有效结果可复用，修改影响验证时只重跑相关检查。
- [x] RT-08 业务秘密、RT-12 业务任务恢复、RT-13 完整格式矩阵未冒充已实现；未通过条件仍未勾选。

**验证：** 核对下方验收映射、完整检查结果及双架构报告；`git diff --check`；`pnpm run format:check`。没有有效结果时执行对应完整检查，不能仅人工阅读后关闭任务。

### 检查点 K10（计划 C5）

- [x] runtime 实际交付、操作说明和剩余边界可供评审。
- [x] 只有对应验收得到证据的任务才能完成；随后按能力地图推进 site 的 Spec。

**本批实施记录（2026-09-16）：** #20–#26 在 PR #46 整合交付，118 项单元、72 项集成、Ego 与原生 AMD64/ARM64 容器/迁移/恢复全部通过。实际发布与后续模块验收仍未执行，详见 [最终证据与保留边界](../runtime-verification.md)。不关闭 Issue、不合并或删除分支。
