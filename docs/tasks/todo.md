...
**实施记录（2026-09-15）：** 已补充 12 项完整入口失败与恢复测试，并与 RUNTIME-09 复用进程辅助函数。真实目录权限、端口占用、SQL 回滚、新旧迁移进度及停止重启后的数据保留均已本地验证；没有修改生产实现或新增业务表。完整检查、审计和远端证据见 [开发记录](../development.md#runtime-10完整入口的失败与恢复)。远端检查完成前保留任务总状态未完成。
