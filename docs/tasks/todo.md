...
- 模块：`runtime`。
- 状态：RUNTIME-01–19 已合入；RUNTIME-20–26 整批实施中。最新结果见 [runtime 验证记录](../runtime-verification.md)，实施状态与实际验收分别维护。
- 日期：2026-09-12。
...
**实施记录（2026-09-15）：** 已补充 12 项完整入口失败与恢复测试，并与 RUNTIME-09 复用进程辅助函数。真实目录权限、端口占用、SQL 回滚、新旧迁移进度及停止重启后的数据保留均已本地验证；没有修改生产实现或新增业务表。完整检查、审计和远端证据见 [开发记录](../development.md#runtime-10完整入口的失败与恢复)。CI 与 AMD64/ARM64 Docker 检查均通过，PR #36 待评审；Issue 保持开放，由用户决定合并和关闭。
...
- [x] 完成实现、本地验证和独立审计；CI 与双架构 Docker 均通过，待 PR 评审。
...
- [x] 完成实现、本地验证与独立审计；CI 和双架构 Docker 均通过，待 PR 评审。
...
**实施记录（2026-09-16）：** 镜像依赖、构建上下文排除和真实样本打包已实现，104 项单元、70 项集成及 Ego Lite 验证通过；独立审计无阻塞发现，CI 与原生 AMD64/ARM64 Docker 均成功，PR #44 待评审。实际命令、样本来源、审计与远端结果见 [开发记录](../development.md#runtime-18实际运行镜像)。

### RUNTIME-19：用真实图片验证镜像工具

- [x] 完成实现、本地验证与独立审计；CI 和双架构真实图片验证通过，待 PR 评审。
