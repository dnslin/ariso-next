...
[Issue #74](https://github.com/dnslin/ariso-next/issues/74) 的本项消费范围为 `UI-LIBRARY` 基础读取。现有两端节点与规则足以表达本切片，文档走查未发现需要补图的具体缺口；真实页面尚未实现，不关闭 DES-06-LIBRARY、DES-05/07 或 RG-07。[核对依据与验证记录](./evidence/DG-LIBRARY-BASE/README.md)。
