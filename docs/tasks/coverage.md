...
- 状态：需求基线与任务归属。具体实施映射见 [mapping](./mapping.md)，任务卡与本地依赖检查见 [索引](./todo.md)；业务尚未实现，不把表格条目当成 Done。
- `R-章节-序号` 是稳定需求 ID；`A-26.x-序号` 是 PRD 原编号验收场景。后续追加条目使用新序号，不随排序重编号。
- 任务组 ID 仅用于拆分范围，不是实施 Issue、可直接开始的任务或全部前置条件。本轮 P2-TASKS 已通过需求映射把每行链接到一个或多个可执行任务；各任务还必须有已评审 Spec、必要设计和真实前置。
...
| R-17.2-02 | 17.2 | 停用存储内容请求报停用，不跨存储/版本回退。 公开的 pending/processing/failed 或停用记录保留不可读占位；可用后恢复原位。               | `SHARING` | 分享内停用图片请求验证    | UI-SHARE-PUBLIC             | 待补 |
| R-17.3-01 | 17.3 | 正确密码授权浏览器24小时；关闭/过期/改密码/重建链接立即失效。                                                                         | `SHARING` | 时间边界及四种撤权验证    | UI-SHARE-PUBLIC             | 待补 |
| R-17.4-01 | 17.4 | 每册独立选网格/瀑布流及隐藏信息/仅displayName。 匿名大图精简，无后台版本切换、技术字段或下载入口。                                    | `SHARING` | 相册独立配置与匿名展示    | UI-SHARING、UI-SHARE-PUBLIC | 待补 |
| R-17.4-02 | 17.4 | 分享不展示原始名/标签/上传时间/存储/EXIF/GPS/大小/技术参数，不提供下载原图按钮；公开原图本身仍可访问。 大图名称服从分享显示名称配置。 | `SHARING` | 响应字段/DOM/原图入口验证 | UI-SHARE-PUBLIC             | 待补 |
...
| 需求 ID   | PRD  | 可验证要求与边界                                                                                                                                                     | 任务组             | 验收方式                       | 设计键 / 无界面理由        | 证据 |
| --------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | -------------------------- | ---- |
| R-19.1-01 | 19.1 | 展示图片/相册数量，各存储Ariso文件占用，含原图、派生和回收站。 正常图库计私有/处理中/失败/停用，回收单列；相册含空册，未交接上传不计图片。                           | `ANALYTICS-REPORT` | 数据/文件清单与界面对账        | UI-DASHBOARD、UI-ANALYTICS | 待补 |
...
| R-19.3-01 | 19.3 | 展示今日/累计、7/30/90天、热门图片、版本量、存储占用图；不统计地区/设备/独立访客/来源。 7/30/90天含今日，趋势/排行/版本量共用周期；今日/累计和当前占用不随周期改变。 | `ANALYTICS-REPORT` | 固定数据查询与图表验证         | UI-DASHBOARD、UI-ANALYTICS | 待补 |
...
| R-19.4-02 | 19.4 | 站点时区每日明细保365天，更早合累计；改时区不改历史日记录，新统计按新时区归档。 永久删除仍保留历史数值；排行以已删除占位且无旧名或内容入口；同名重传不继承。         | `ANALYTICS-COUNT`  | 365日边界/时区变更历史验证     | 无界面：统计保留           | 待补 |
...
每项均为待验证，场景内容保持 PRD 编号。P2-ACCEPTANCE 已把这些行落实到切片与首版验收任务；相关任务组实现与验收前置满足后，由 `QUALITY-INTEGRATION` 组织全流程验证。每行的要求 ID 同时连接上文的具体规则和方法，不用一个章节级勾选代表所有场景通过。 “必需业务组”按该场景本身列出，不能把所引要求行的全部责任组合并成前置；例如本地上传不等待 S3，相册分享和底层 S3 私有性分别验收，不阻塞 Ariso 版本鉴权。组内具体任务见任务索引，前置以实际被调用能力为准。
...
- 每条要求和场景的实施任务 ID 已见 [自动生成映射](./mapping.md)；GitHub Issue/PR 与实际执行结果待实施时补充，本轮没有创建 Issue。
- 设计列保留稳定家族键；具体任务卡已关联两端主/状态节点、HeroUI及设计适用核对：DES-01 初始化、DES-02 重置落地、DES-03 匿名分享、DES-04 相册管理/封面、DES-05 深色、DES-06 各功能状态、DES-07 应用外壳。未完成设计只阻塞对应界面，不借用其他页面节点作为已覆盖证明。
