...
macOS arm64、Linux AMD64 和 ARM64 实验三项验收通过：Unicode 规则符合规格；同名相册保持独立；重复标签和并发竞争不产生第二条。独立审计无必须修复项；双架构实验和完整 Docker 工作流均通过。既有身份页面浏览器回归失败；用户于 2026-09-22 明确要求本次不处理该问题并继续完整流程，因此该范围外失败不阻塞本 Issue 待评审，不把部分回归成功表述为全部通过。
...
该检查及对应生产界面与分支基线相同，本次只新增实验和开发依赖。此范围外问题保留给所属身份/外壳任务；按用户明确指示，不再作为本 PR 转为正式待评审的阻塞项。生产 collections、上传关系、管理界面、跨浏览器矩阵仍不属于本前置实验。

最终只补充远端证据及本报告，不改已通过验证的实验代码。文档格式检查与本地相对链接检查通过；PR 实时状态和最新检查结果以 [#102](https://github.com/dnslin/ariso-next/pull/102) 为准。未合并、未关闭 Issue、未发布或部署，分支与 worktree 保留。
