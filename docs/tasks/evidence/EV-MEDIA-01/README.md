...
当前状态：本地验证进行中，真实容器结果尚待 Actions，不能按脚本存在认定验收通过。
...
- 读取当前镜像 policy/resource，不覆盖已有配置；拒绝镜像遗留 width/height/list-length policy。真实 32769×1 PNG 转 640×1 验证没有历史 16K/32K 宽度准入。此样本不是任意尺寸、动画或多页可处理的承诺。
