...
本节状态表及 `/i/{id}` 规则描述外链入口；下方单列用户批准的所有者管理预览。每请求读取 identity 的真实会话。有效所有者 Cookie 才是所有者；上传 Bearer Token、分享密码/授权和客户端 userId 均不构成图片访问身份。Cookie 无效/过期按匿名处理；认证数据库或验证服务异常必须报错，不能把真实故障悄悄当匿名后计数。
...
### 所有者回收管理预览（2026-09-26 用户批准）

回收列表缩略图和记录详情预览使用 `GET/HEAD /api/trash/{id}/preview?type=thumbnail|original|compressed|watermark`，显式指定版本且不接受下载参数。该独立管理预览入口仅有效所有者 Cookie 可读；不以 API Token、分享授权或 public 属性替代身份。入口限定当前回收、尚未永久删除且存储启用的图片及已保存实际版本，不接受任意路径，不生成新版本。deleting、cleanup_failed、存储停用、版本缺失和无效会话明确拒绝；准备文件或签名的异步间隔沿现有授权复核方式检查这些状态。响应沿现有 no-store、传输和错误约定，不计外链访问量。管理预览 URL 不作为可复制的公开链接，也不新增下载入口。原 `/i/{id}` 及全部显式版本对回收图片仍拒绝所有人，包括所有者。
...
| UI-TRASH        | [30:1037](https://www.figma.com/design/74sT9Hrf8G4czcWeTkET5b?node-id=30-1037) / [102:852](https://www.figma.com/design/74sT9Hrf8G4czcWeTkET5b?node-id=102-852) | 回收后仅所有者管理预览可读取缩略图/已有版本；原外链仍拒绝                     |
...
2026-09-26 用户批准回收站列表缩略图及详情管理预览，按第 4 节独立入口授权，不放宽 `/i` 外链。原画板“不显示内容”由此项明确批准覆盖，不修改 Figma；已显示字节仍不能撤回。可见页面的完整布局、选择和批量反馈仍由 library/upload/sharing 规格负责。
...
| DL-06 | 回收/永久删除外链所有者也不可读；管理预览仅所有者可读回收图；恢复同链接、停用拒绝      | R-18.1-01、R-18.2-01、A-26.11-01、A-26.11-02、A-26.11-04 |
...
| DL-18 | 桌面/手机实际版本、错误、原图信息提示和 SVG 预览正确；回收管理预览独立鉴权且不放开外链 | R-13.4-02、R-14.9-01、R-18.1-01；PRD 22                  |
...
2. **处理失败与回收站**：失败图片的已保存原图可由所有者下载；移入回收站后所有外链关闭；有效所有者可通过独立管理预览读取已保存版本，停用或删除中仍占位。
